...
if (!empty($_POST)){
    ...
    $cus_name = ($data[count($data)-3]);
    $cus_phon = ($data[count($data)-2]);
    $cus_mail = ($data[count($data)-1]);
    ...
    //insert order details    
        $stmt = $pdo->exec("INSERT INTO orders (id, customer_phone, customer_name, customer_mail, order_cost, order_date,status) VALUES 
        ('".$order_id."','".$cus_phon."','".$cus_name."','".$cus_mail."','".$order_cost."','".$order_date."','".$order_status."')");
